...
if __name__ == '__main__':
    ...
    # imgname = 'csail-mit.jpg'
    imgname = 'doggie-corgi_wallpaperaccess-com1.jpg'
    
    cv2_img = get_img(os.path.join(IMG_DIR, imgname))
    cv2_img = normalize_img(cv2_img)
    print(cv2_img.shape)
    show_img(pooling(cv2_img, pool_size=2, stride=2, pad=0, pool_type='max'))

    # cv2_sobel = get_sobel(cv2_img, 'scharr')
    # show_img(cv2_sobel)
    
    output = convolution(cv2_img, get_kernel('canny_edge_detect'), stride=1, pad=0)
    print(output.shape)
    # show_img(output)
    ...
    # standard vs softmax: affect when values is large
    # arr = np.array([0.5, 1.0, 3.0])*10  # *1 vs *10
    # print(arr/np.sum(arr, axis=0))  # standard normalize
    # print(np.exp(arr)/np.sum(np.exp(arr), axis=0))  # softmax